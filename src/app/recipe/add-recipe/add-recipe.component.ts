...
export class AddRecipeComponent {
  ...
  addRecipe() {
    if (!this.recipeName || !this.recipeIngredients.length) {  // if we try to add recipe without name or not adding atleast one ingredient
      this.errMsg =
        'add recipe name and recipe must have at least one ingredient';
      return;
    }
    if (this.selectedId) {   //editing existing recipe
